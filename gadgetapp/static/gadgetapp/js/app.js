    ...
    $scope.getData = function(){
        ...
        RestaurantService.getRestaurantList(function (data) {
            if (data != null){
                ...
                $scope.pending = ($scope.order_items.filter(function(item) { return (item.order_status == 'P' || item.order_status == 'A');}));
                $scope.delivered = ($scope.order_items.filter(function(item) { return (item.order_status == 'D');}));
                $scope.rejected = ($scope.order_items.filter(function(item) { return (item.order_status == 'RE');}));
